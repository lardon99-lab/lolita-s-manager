...
// 3. MANEJO DEL FORMULARIO DE ABASTECER
document.addEventListener('submit', function(e) {
    if (e.target && e.target.classList.contains('formAbastecer')) {
        e.preventDefault(); 
        ...
        fetch('api.php?resource=inventario&action=abastecer', {
            method: 'POST',
            body: new FormData(form)
        })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                ...
                if (btnSubmit) {
                    btnSubmit.disabled = false;
                    btnSubmit.innerHTML = 'Confirmar Ingreso'; 
                }
            }
        })
        .catch(error => {
